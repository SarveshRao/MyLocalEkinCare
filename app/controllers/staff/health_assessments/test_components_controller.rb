class Staff::HealthAssessments::TestComponentsController < StaffController
  ...
  def updateStandardRange
    ...
    if(params['columnPosition'] == "0")
      ...
      temp_standard_range_array[params['columnPosition'].to_i - 3] = params['value']
  ...
  def addTestComponent
    ...
    @insertion_row_tc=insert_test_component(test_component_lab_test_id, test_component_name, test_component_unit, test_component_info, enterprise_id)
  ...
  protected
  def insert_test_component (test_component_lab_test_id, test_component_name, test_component_unit, test_component_info, enterprise_id)
    inserted_row_id = -1
    begin
      if(test_component_unit == '')
        inserted_row_id = TestComponent.create(lab_test_id: test_component_lab_test_id, name:test_component_name, info: test_component_info, enterprise_id: enterprise_id)
      else
        inserted_row_id = TestComponent.create(lab_test_id: test_component_lab_test_id, name:test_component_name, units:test_component_unit, info: test_component_info, enterprise_id: enterprise_id)
      end
    end
