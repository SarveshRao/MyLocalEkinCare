...
//  var Notes = new NoteList;
    ...
    var NoteItemView = Backbone.View.extend({
        ...
        template: _.template($('#item-multi-template').html()),
...
//      window.history.back();
        ...
        select: function () {
            this.active();
            Backbone.history.loadUrl("notes/" + this.model.get('id'));
        },
    ...
    var NoteListView = Backbone.View.extend({
        ...
        events: {
            "click #new-multi-note": "createNote",
            "keyup #search-multi-note": "search"
        },
...
//      this.listenTo(Notes, 'add', this.addOne);
        ...
        addOne: function (note) {
            ...
            this.$el.find('#note-multi-items').prepend(view.render().el);
        },
        ...
        createNote: function (e) {
            ...
            var note = new Note({created_time: today, updated_time: today});
            Notes.add(note);
//      note.url = '/health_assessments/'+assessment_id+'/notes'
            note.save({}, {url: '/health_assessments/' + assessment_id + '/notes', success: function (model, response) {
                _this.addOne(model);
            }});
        },

        search: function () {
            Notes.search($('#search-multi-note').val());
        }
    ...
    var NoteView = Backbone.View.extend({
        el: $("#note-multi-detail"),

        // Cache the template function
        template: _.template($('#note-multi-template').html()),
        ...
        updateOnKeyup: function (e) {
            ...
            arr.length && ( name = _.first(_.filter(arr, function (item) {
                return !!$.trim(item);
            })) );
    ...
    var AppMultiRouter = Backbone.Router.extend({
        routes: {
//      "nts": "list",
            "notes/:id": "details"
        },
        ...
        list: function (options) {
            ...
            Notes = new NoteList({id: options.id})
            assessment_id = options.id;
            this.noteListView = new NoteListView;
            var self = this;
            Notes.fetch({
                success: function (collection, response) {
                    Notes = collection
//                    $("#note-multi-detail").html(new NoteView({model: Notes.first()}))
                    Notes.trigger('reset');
                    Notes.length && self.details(assessment_id) && Backbone.history.loadUrl('/notes/' + assessment_id);
                }
            });
        },

        details: function (id) {
//      if ($.inArray('active', $('#notes').parent().attr('class')) == -1) {
