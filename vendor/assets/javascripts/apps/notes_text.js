...
//    },
    ...
    list: function (assessment_id) {
//      if (this.noteListView) return;
//      if (typeof options == 'undefined') return;
      Notes = new NoteList({id: assessment_id})
//      this.noteListView = new NoteListView;
      var self = this;
      Notes.fetch({
        success: function (collection, response) {
          Notes = collection
//          $("#note-detail").html(new NoteView({model: Notes.first()}))
//          Notes.trigger('reset');
          this.note = collection.last();
//          if (this.note) {
//            this.note.trigger('active');
          this.noteView = new NoteView({model: this.note});
          this.noteView.render();
//          }
//          Notes.length && self.details(assessment_id) && app.navigate('notes/' + assessment_id);
        }
      });
    }
...
//    }
  ...
  if($('#notes')){
//    app.list($('#notes').attr('assessment_id'))
//    alert($('#notes').attr('assessment_id'))
  }
